...
//	      goog.bind(this.scene.updateCopyrights, this);
	...
	private void updateTiles_() {
		  ...
		  this.offset[0] = (int) Math.floor(cameraTarget[1] / (2 * Math.PI) * this.tileCount);
		  this.offset[1] = (int) Math.floor(Scene.projectLatitude(cameraTarget[0]) /
	...
	public void draw() {
		this.updateTiles_();
		  ...
		  GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackA_.getBuffer(zoom, 0));
		  GLES20.glUniform1i(this.locatedProgram.bufferL0Uniform, 0);

		  GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
		  GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackA_.getBuffer(zoom, 1));
		  GLES20.glUniform1i(this.locatedProgram.bufferL1Uniform, 1);

		  GLES20.glActiveTexture(GLES20.GL_TEXTURE2);
		  GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackA_.getBuffer(zoom, 2));
		  GLES20.glUniform1i(this.locatedProgram.bufferL2Uniform, 2);

		  GLES20.glActiveTexture(GLES20.GL_TEXTURE3);
		  GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackA_.leveln.texture);
		  GLES20.glUniform1i(this.locatedProgram.bufferLnUniform, 3);

		  GLES20.glUniform1fv(this.locatedProgram.metaL0Uniform,this.clipStackA_.getMeta(zoom, 0).length,
		                this.clipStackA_.getMeta(zoom, 0),0);
		  GLES20.glUniform1fv(this.locatedProgram.metaL1Uniform,this.clipStackA_.getMeta(zoom, 1).length,
		                this.clipStackA_.getMeta(zoom, 1),0);
		  GLES20.glUniform1fv(this.locatedProgram.metaL2Uniform,this.clipStackA_.getMeta(zoom, 2).length,
		                this.clipStackA_.getMeta(zoom, 2),0);

		  GLES20.glUniform2fv(this.locatedProgram.levelOffsetsUniform,this.clipStackA_.getOffsets(zoom, 3).length,
		                this.clipStackA_.getOffsets(zoom, 3),0);

		  if (this.terrain) {

		    int terrainZoom = (int) org.earth.Utils.clamp(zoom - TERRAIN_ZOOM_DIFFERENCE,
		                                      2,
		                                      this.terrainProvider_.getMaxZoomLevel());

		    GLES20.glUniform1f(this.locatedProgram.degradationTUniform, zoom - terrainZoom);

		    GLES20.glActiveTexture(GLES20.GL_TEXTURE4);
		    GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackT_.getBuffer(terrainZoom, 0));
		    GLES20.glUniform1i(this.locatedProgram.bufferL0TUniform, 4);

		    GLES20.glActiveTexture(GLES20.GL_TEXTURE5);
		    GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackT_.getBuffer(terrainZoom, 1));
		    GLES20.glUniform1i(this.locatedProgram.bufferL1TUniform, 5);

		    GLES20.glActiveTexture(GLES20.GL_TEXTURE6);
		    GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.clipStackT_.leveln.texture);
		    GLES20.glUniform1i(this.locatedProgram.bufferLnTUniform, 6);

		    GLES20.glUniform1fv(this.locatedProgram.metaL0TUniform,this.clipStackT_.getMeta(terrainZoom, 0).length,
		                  this.clipStackT_.getMeta(terrainZoom, 0),0);
		    GLES20.glUniform1fv(this.locatedProgram.metaL1TUniform,this.clipStackT_.getMeta(terrainZoom, 1).length,
		                  this.clipStackT_.getMeta(terrainZoom, 1),0);

		    GLES20.glUniform2fv(this.locatedProgram.levelOffsetsTUniform,this.clipStackT_.getOffsets(terrainZoom, 2).length,
		    			  this.clipStackT_.getOffsets(terrainZoom, 2),0);
		  }

		  float [] mvpm = Utils.transposeMatrix(this.context.flushMVPM());

		  SegmentedPlane plane = this.segPlanes_[Math.min(zoom, this.segPlanes_.length - 1)];

		  AndroidGL20 agl20 = new AndroidGL20();
		  
		  GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, plane.vertexBuffer.bufferId);
		  agl20.glVertexAttribPointer(this.locatedProgram.vertexPositionAttribute,
		      plane.vertexBuffer.itemSize,
		      GLES20.GL_FLOAT, false, 0, 0);

		  GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, plane.texCoordBuffer.bufferId);
		  agl20.glVertexAttribPointer(this.locatedProgram.textureCoordAttribute,
		      plane.texCoordBuffer.itemSize,
		      GLES20.GL_FLOAT, false, 0, 0);

		  GLES20.glUniformMatrix4fv(this.locatedProgram.mvpMatrixUniform, mvpm.length, false, mvpm, 0);
		  GLES20.glUniform1f(this.locatedProgram.tileCountUniform, this.tileCount);

		  GLES20.glUniform2fv(this.locatedProgram.offsetUniform,this.offset.length,
		                this.offset,0);

		  GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, plane.indexBuffer.bufferId);
		  //if (Math.floor(goog.now() / 10000) % 2 === 1)
		  agl20.glDrawElements(GLES20.GL_TRIANGLES, plane.numIndices, GLES20.GL_UNSIGNED_SHORT, 0);
